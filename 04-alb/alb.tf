...
# Listener rule for v2 path - commented out, available for future use
...
resource "aws_autoscaling_attachment" "app_v1" {
  autoscaling_group_name = data.terraform_remote_state.app_ebs_v1.outputs.beanstalk_asg_names[0]
  lb_target_group_arn    = aws_lb_target_group.app_v1.arn
}

# Note: app_v2 target group available for future use if 03-ebs-v2 is deployed
